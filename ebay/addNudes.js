...
if (username === "enycez-collections") {
  EBAY_AUTH_TOKEN = fs.readFileSync("/Users/wkchris/Desktop/keys/enycez-collections-key.txt");
  data = {
    photosTable: "nudes",
    locationZipCode: 10708,
    paymentId: pid,
    returnId: 224521479025,
    shippingId: 224521462025,
...
function generateAddItemsRequest(products) {
  const items = products.map((product) => {
    ...
    const { title, imgUrl, isAdult, SKU, description, categoryId, notes } =
      product;
    const isNonNude = notes === "nonnude";

    let nudeTag =
      Math.random() < 0.35 ? "<Value>Nudes.</Value>" : "<Value>Nudes</Value>";
...
<h1>
<b>Glossy 8x10 Collectible Artistic Photographs</b>
<br>
<br>
<font color="cyan">
<b>All orders will be shipped securely to prevent damage and discreetly to ensure complete privacy.</b>
<b>All orders will come provided with USPS tracking number.</b>
</font>
<br>
<br>
<font color="red">
<h3><i>All Listings will be private to ensure complete privacy.</i></h3>
</font>
<br>
<font color="pink">
...
<br>
<font color="brown">
<b><i>All models are 18 years or older.</i></b>
<br>
<b><i>Purchase of any and all photos does not give or imply a transfer of rights.</i></b>
<br>
<b>All photos are strictly sold from collector to collector.</b>
<br>
<br>
<center>THANK YOU FOR BROWSING! 😄</center>
</div>`;
...
function getPhotosToCreateListing(connection, isNude) {
  return new Promise((resolve, reject) => {
    connection.query(
      ...
      `SELECT *,CONCAT('N',LPAD(CONV(row_id,10,36), 6, 0)) as SKU FROM ${data.photosTable} WHERE ebay_item_number IS NULL AND isNude = ? AND ebay_pic_url IS NOT NULL;`,
      [isNude],
...
function createListingDataForPhoto(photo) {
  ...
  let title = `8x10 ${titleWords
    ...
    .join(" ")} ${SKU}`;
  if (photo.model_name) {
    title = `8x10 Glossy Photo Beautiful Nude Model Pinup Girl (Model ${photo.model_name}) ${SKU}`;
  } else if (photo.notes === "FineArt") {
    title = `8x10 Glossy Photo Fine Art Beautiful Sexy Female Model  ${SKU}`;
  } else if (photo.notes === "nonnude") {
    title = `8x10 Glossy Photo Sexy Beautiful Model NN Pinup Girl ${SKU}`;
  }
  if (title.length > 80) {
